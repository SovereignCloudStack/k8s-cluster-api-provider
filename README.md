...
`etcd_prio_boost` | `ETCD_PRIO_BOOST` | SCS | `true` | Longer heartbeat and high CPU share for etcd in case you don't have dedicated cores
`etcd_unsafe_fs` | `ETCD_UNSAFE_FS` | SCS | `true` | Use `barrier=0` for filesystem on control nodes to avoid storage latency. Unsafe when the machine crashes, but keeps etcd clusters more stable.
...
`deploy_nginx_ingress` | `DEPLOY_NGINX_INGRESS` | SCS | `true` | `apply_nginx_ingress.sh` | Deploy NGINX ingress controller (this spawns an OpenStack Loadbalancer), pass version to explicitly choose the version, `true` results in `v1.1.2` (`v1.0.2` for k8s <= 1.19)
