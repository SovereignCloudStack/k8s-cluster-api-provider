...
`dns_nameservers` | `OPENSTACK_DNS_NAMESERVERS` | capo | `[ "5.1.66.255", "185.150.99.255" ]` | Array of nameservers for capi mgmt nodes and for cluster nodes
...
`anti_affinity` | `OPENSTACK_ANTI_AFFINITY` | SCS | `false` | Use anti-affinity server groups to prevent k8s nodes on same host (soft for workers, hard for controllers)
