...
if test "$NGINX_INGRESS_PROXY" = "false"; then
	kubectl kustomize nginx-monitor > ~/$CLUSTER_NAME/deployed-manifests.d/nginx-ingress.yaml || exit 3
else
	kubectl kustomize nginx-proxy > ~/$CLUSTER_NAME/deployed-manifests.d/nginx-ingress.yaml || exit 3
fi
