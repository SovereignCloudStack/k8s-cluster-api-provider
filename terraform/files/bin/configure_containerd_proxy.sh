...
file_cp_exist=$(yq 'select(.kind == "KubeadmControlPlane").spec.kubeadmConfigSpec | (.files // (.files = []))[] | select(.path == "/etc/systemd/system/containerd.service.d/http-proxy.conf")' "$1")
...
if test -z "$file_cp_exist"; then
	...
	yq 'select(.kind == "KubeadmControlPlane").spec.kubeadmConfigSpec.files += [load(env(CLUSTER_TEMPLATE_SNIPPET))]' -i "$1"
        ...
        echo "containerd proxy config is already defined in KubeadmControlPlane files"
fi
