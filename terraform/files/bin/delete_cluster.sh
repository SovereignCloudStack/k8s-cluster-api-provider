...
kubectl --context=$KCONTEXT get nodes -o=jsonpath="{'set -x\n'}{range .items[*]}{'kubectl --context=$KCONTEXT cordon '}{.metadata.name}{'\n'}{end}" | bash
# Delete storage classes (prevents creation of new PVs)
echo "Deleting storage classes ..."
kubectl --context=$KCONTEXT get storageclasses -o=jsonpath="{'set -x\n'}{range .items[*]}{'kubectl --context=$KCONTEXT delete storageclass '}{.metadata.name}{' --ignore-not-found=true\n'}{end}" | bash
# Delete nginx ingress
INPODS=$(kubectl --context=$KCONTEXT --namespace ingress-nginx get pods)
...
kubectl --context=$KCONTEXT get daemonsets --all-namespaces -o=jsonpath="{'set -x\n'}{range .items[?(@.spec.template.spec.volumes[*].persistentVolumeClaim.claimName)]}{'kubectl --context=$KCONTEXT delete daemonset '}{.metadata.name}{' -n '}{.metadata.namespace}{' --grace-period=0 --ignore-not-found=true\n'}{end}" | bash
# Delete statefulsets with persistent volume claims
echo "Deleting statefulsets with persistent volume claims ..."
kubectl --context=$KCONTEXT get statefulsets --all-namespaces -o=jsonpath="{'set -x\n'}{range .items[?(@.spec.template.spec.volumes[*].persistentVolumeClaim.claimName)]}{'kubectl --context=$KCONTEXT delete statefulset '}{.metadata.name}{' -n '}{.metadata.namespace}{' --grace-period=0 --ignore-not-found=true\n'}{end}" | bash
# Delete all CronJobs
...
kubectl --context=$KCONTEXT get services --all-namespaces --field-selector metadata.name!=kubernetes -o=jsonpath="{'set -x\n'}{range .items[*]}{'kubectl --context=$KCONTEXT delete service '}{.metadata.name}{' -n '}{.metadata.namespace}{' --grace-period=0 --ignore-not-found=true\n'}{end}" | bash
...
KUBECONFIG=$KUBECONFIG_ORIG kubectl config delete-context "$CLUSTER_NAME-admin@$CLUSTER_NAME"
KUBECONFIG=$KUBECONFIG_ORIG kubectl config delete-user "$CLUSTER_NAME-admin"
KUBECONFIG=$KUBECONFIG_ORIG kubectl config delete-cluster "$CLUSTER_NAME"
