...
usage()
{
	echo "Usage: update-cluster-template-to-generation-counted.sh CLUSTERNAME"
...
# cluster-template
patch -R --dry-run cluster-template.yaml < ~/k8s-cluster-api-provider/terraform/files/update-cluster-template.diff >/dev/null 2>&1
if test $? == 0; then
	echo "cluster-template.yaml already upgraded" 1>&2
else
	patch cluster-template.yaml < ~/k8s-cluster-api-provider/terraform/files/update-cluster-template.diff || restore 4
fi
...
if grep '^CONTROL_PLANE_MACHINE_GEN' clusterctl.yaml >/dev/null 2>&1; then
	...
	sed -i -f ~/k8s-cluster-api-provider/terraform/files/update-clusterctl-control-gen.sed clusterctl.yaml || restore 5
fi
# WORKER_MACHINE_GEN
if grep '^WORKER_MACHINE_GEN' clusterctl.yaml >/dev/null 2>&1; then
	echo "WORKER_MACHINE_GEN already set in clusterctl.yaml" 1>&2
else
	sed -i -f ~/k8s-cluster-api-provider/terraform/files/update-clusterctl-worker-gen.sed clusterctl.yaml || restore 6
fi
